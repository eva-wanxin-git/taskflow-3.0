...
class TaskStatusPoller:
    ...
    def update_status(self, cursor, task_id, new_status):
        ...
        project_id = row[1] if row and len(row) > 1 else None
    ...
    def _emit_status_changed_event(self, project_id, task_id, old_status, new_status):
        """将自动状态纠偏写入 project_events。"""
        if not getattr(self, "_event_emitter", None):
            return
        if not project_id or not old_status or old_status == new_status:
            return
        ...
        try:
            self._event_emitter.emit(
                ...
                data={
                    ...
                    "old_status": old_status,
                    "new_status": new_status,
                    "source": "task_status_poller",
                },
            )
        except Exception as e:
            print(f"[ERROR] 发射状态变更事件失败: {e}")

...
SESSION_MEMORY_URL = "http://13.158.83.99:4000"  # Session Memory HTTP API
TIMEOUT = 10
...
async def sync_to_session_memory(session_id: str) -> Dict[str, Any]:
    ...
    **返回**:
    ...
    try:
        ...
        # 构建Session Memory HTTP API格式的数据
        session_memory_data = {
            "namespace": "taskflow_sessions",  # 任务所会话专用namespace
        ...
        # 调用Session Memory HTTP API
        async with httpx.AsyncClient(timeout=TIMEOUT) as client:
            response = await client.post(
                f"{SESSION_MEMORY_URL}/memory/store",  # 正确的API路径

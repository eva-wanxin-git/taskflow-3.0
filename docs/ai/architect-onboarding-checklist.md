# ✅ 架构师接手项目检查清单

**版本**: v1.0  
**更新时间**: 2025-11-18

---

## 🎯 架构师接手新项目的标准流程

当你被任命为某个项目的架构师时，**第一件事**是为项目分配独立端口。

---

## 📍 Step 0: 端口分配（最优先）

### 0.1 自动分配端口

**在开始分析项目之前，先执行**：

```python
# 方式1：在Python中
import sys
sys.path.insert(0, "path/to/taskflow/packages/shared-utils")
from port_manager import allocate_project_port

# 为项目分配端口
port = allocate_project_port("MY_PROJECT")
print(f"✓ 项目 MY_PROJECT 分配端口: {port}")
```

**或使用命令行**：
```bash
# 在任务所Flow目录中
cd taskflow-v1.7-monorepo
python -c "
from packages.shared_utils.port_manager import allocate_project_port
port = allocate_project_port('MY_PROJECT')
print(f'端口: {port}')
"
```

### 0.2 记录端口到项目配置

**在项目根目录创建** `.taskflow.yaml`:
```yaml
# 任务所·Flow配置
taskflow:
  api_url: "http://localhost:{port}"  # 使用分配的端口
  project_code: "MY_PROJECT"
  dashboard_port: {port}
  auto_sync: true

# 项目信息
project:
  name: "我的项目"
  description: "项目描述"
  repo_url: "https://github.com/xxx/my-project"
```

### 0.3 在任务板中声明端口

**在`docs/tasks/task-board.md`开头添加**：
```markdown
**📍 端口信息**:
- **Dashboard端口**: {port}（自动分配）
- **访问地址**: http://localhost:{port}
- **端口范围**: 8870-8899（任务所Flow专用）
- **端口管理**: 通过PortManager自动分配，避免冲突
```

---

## 📍 Step 1-4: 正常的架构师流程

[继续执行architect-system-prompt中的标准流程]

1. 轻量扫描
2. 深度审查
3. 生成4份文档
4. 拆解任务

---

## 📍 Step 5: 打开任务看板（让用户看到）

### 生成并打开HTML任务板

**执行脚本**：
```bash
cd taskflow-v1.7-monorepo
python open_task_board.py
```

**脚本会自动**：
1. ✅ 为项目分配/获取端口
2. ✅ 读取Markdown任务板
3. ✅ 生成HTML版本
4. ✅ 在浏览器中打开

**用户立即可以看到**：
- 项目端口信息
- 任务进度统计
- 详细任务列表
- 优先级和依赖关系

---

## 🔄 架构师接手项目的完整Checklist

### ✅ 前置工作（5分钟）

- [ ] **分配端口** - `allocate_project_port("PROJECT_CODE")`
- [ ] **创建配置** - 生成`.taskflow.yaml`
- [ ] **记录到文档** - 在task-board.md中声明端口

### ✅ 项目分析（10-20分钟）

- [ ] **轻量扫描** - 目录结构、配置文件、README
- [ ] **深度审查** - 10-20个核心文件
- [ ] **生成文档** - inventory/review/refactor-plan/task-board

### ✅ 任务拆解（5-10分钟）

- [ ] **整理清单** - ✅已实现 🟡部分实现 🔴问题 🧹任务
- [ ] **创建任务** - 拆解为可执行任务
- [ ] **设置优先级** - P0/P1/P2/P3

### ✅ 同步和展示（2分钟）

- [ ] **提交API** - `POST /api/architect/analysis`（如有任务所）
- [ ] **打开任务板** - `python open_task_board.py`
- [ ] **用户确认** - 等待用户查看和反馈

---

## 📊 端口分配规则

### 端口范围
```
8870-8899: 任务所Flow专用（30个端口）

8870: 任务所Flow本身
8871: 第1个接入项目
8872: 第2个接入项目
...
8899: 第30个接入项目
```

### 端口状态
- **active**: 正在使用
- **released**: 已释放（可重新分配）

### 端口配置文件
```
taskflow-v1.7-monorepo/config/ports.json

{
  "version": "1.0",
  "port_range": {"start": 8870, "end": 8899},
  "allocated_ports": {
    "TASKFLOW": {
      "port": 8870,
      "allocated_at": "2025-11-18T22:30:00",
      "status": "active"
    },
    "MY_PROJECT": {
      "port": 8871,
      "allocated_at": "2025-11-18T22:35:00",
      "status": "active"
    }
  }
}
```

---

## 🛠️ 实用命令

### 查看端口分配情况
```bash
cd taskflow-v1.7-monorepo
python packages/shared-utils/port_manager.py
```

### 为项目分配端口
```python
from packages.shared_utils.port_manager import allocate_project_port
port = allocate_project_port("MY_PROJECT")
```

### 释放端口
```python
from packages.shared_utils.port_manager import PortManager
manager = PortManager()
manager.release_port("OLD_PROJECT")
```

---

## 💡 最佳实践

### 1. 项目命名规范
```
推荐：全大写+下划线
- TASKFLOW
- MY_PROJECT
- LIBRECHAT_DESKTOP

避免：
- my-project（连字符）
- MyProject（驼峰）
- 我的项目（中文）
```

### 2. 端口检查时机
```
接手项目时：立即分配端口
更新任务板后：立即打开浏览器
定期检查：每周查看端口使用情况
```

### 3. 端口冲突处理
```
如果端口被占用：
1. PortManager会自动跳过
2. 分配下一个可用端口
3. 记录到ports.json
4. 更新task-board.md
```

---

**检查清单版本**: v1.0  
**最后更新**: 2025-11-18  
**状态**: ✅ 生产就绪

✅ **架构师接手项目的标准化流程！**

